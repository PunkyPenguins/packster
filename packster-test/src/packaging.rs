...
use packster_core::{
    application::{
        operation::Operation,
        path::Absolute,
        port::{Digester, FileSystem, ReadOnlyFileSystem, UniqueIdentifierGenerator},
    },
    domain::entity::Checksum,
    packaging::{application::*, LOCKFILE_NAME},
    Error, Result,
};

use packster_infrastructure::{InMemoryFileSystem, Json, Sha2Digester, TarballArchiver, Toml};
...
fn test_static_packing() -> Result<()> {
    ...
    assert_eq!(
        filesystem_as_archiver.read_to_string("hello_world.txt")?,
...
fn test_init_location_initialization_case() -> Result<()> {
    ...
    let request =
        InitLocationRequest::new(Absolute::assume_absolute(PathBuf::from("/my/location")));
    Operation::new(request).initialize_lockfile(&filesystem, &Json)?;
    ...
    assert_eq!(
        filesystem.read_to_string(expected_lockfile_path)?,
        expected_lockfile_content
    );
...
fn test_deployment_new_package() -> Result<()> {
    ...
    filesystem
        .open_write(&lockfile_path)?
        .write_all(empty_lockfile_content.as_bytes())
        .unwrap();
    ...
    assert!(filesystem
        .exists("/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4"));
    assert!(filesystem.is_directory(
        "/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4"
    ));
    ...
    assert!(filesystem.exists(
        "/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4/a_file.txt"
    ));
    ...
    assert!(filesystem.exists(
        "/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4/a_directory"
    ));
    assert!(filesystem.is_directory(
        "/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4/a_directory"
    ));
    ...
    assert!(lockfile_content
        .contains("d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4"));
...
fn test_deployment_already_existing_package() -> Result<()> {
    ...
    filesystem
        .open_write(&lockfile_path)?
        .write_all(lockfile_value.to_string().as_bytes())
        .unwrap();
    ...
    let result = Operation::new(request)
        .parse_package_path()?
        .parse_location_lockfile(&filesystem, &Json)?
        .probe_package_not_deployed_in_location();
    ...
    assert!(matches!(
        result,
        Err(Error::PackageAlreadyDeployedInLocation(_))
    ));
...
fn test_init_location_lockfile_present_error_case() -> Result<()> {
    ...
    let request =
        InitLocationRequest::new(Absolute::assume_absolute(PathBuf::from("/my/location")));
    let result = Operation::new(request).initialize_lockfile(&filesystem, &Json);

    assert!(matches!(
        result,
        Result::Err(Error::AlreadyPresentLockfile(_))
    ));
...
fn test_undeploy_existing_deployment() -> Result<()> {
    let filesystem = InMemoryFileSystem::default();
    filesystem.create_dir_recursively(
        "/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4",
    )?;
    ...
    assert!(!filesystem.exists("/my/location/d829752c10db8f7a98c939b5418beb0a360c6a6b818830e000f2c5a8dce35af4/packster.toml"));
    assert_eq!(
        filesystem.read_to_string(lockfile_path)?,
        r#"{"deployments":[]}"#
    );
...
fn test_undeploy_not_existing_deployment() -> Result<()> {
    ...
    assert!(matches!(
        result,
        Result::Err(Error::PackageNotYetDeployedInLocation(_))
    ));
