...
where
    Self: AsPackage + AsLocation,
{
    pub fn probe_package_not_deployed_in_location(self) -> Result<Operation<NotYetDeployed<S>, R>> {
        if self
            .as_location()
            .is_checksum_deployed(self.as_package().as_checksum())
        {
            Err(Error::PackageAlreadyDeployedInLocation(
                self.as_package().as_identifier().to_string(),
            ))
        } else {
            Self::ok_with_state(
                self.request,
...
pub struct AlreadyDeployed<S> {
    pub previous_state: S,
    pub existing_deployment: Deployment,
}

impl<S, R> Operation<S, R>
where
    Self: AsChecksum + AsLocation,
{
    pub fn probe_package_already_deployed_in_location(
        self,
    ) -> Result<Operation<AlreadyDeployed<S>, R>> {
        if let Some(existing_deployment) = self
            .as_location()
            .get_deployment(self.as_checksum())
            .cloned()
        {
            Self::ok_with_state(
                self.request,
            ...
            Err(Error::PackageNotYetDeployedInLocation(
                self.as_checksum().to_string(),
            ))
        }
    }
}
...
pub struct DeploymentPath<S> {
    pub previous_state: S,
    pub deployment_path: Absolute<PathBuf>,
}

impl<S, R> Operation<S, R>
where
    Self: AsChecksum + AsLocationPath,
{
